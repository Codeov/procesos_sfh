...
def procesar_fpinterno_df(fecha_i,fecha_f,cod_local,obj_ora_s,q_fpi):
    query_trx_fpi=r_q_fp_interno(fecha_i,fecha_f,cod_local)
...
def procesar_fpefectivo_df(fecha_i,fecha_f,cod_local,obj_ora_s,q_fpef):
    query_trx_fpef=r_q_fp_efectivo(fecha_i,fecha_f,cod_local)
...
#procesar_fpvale_df
def procesar_fpvale_df(fecha_i,fecha_f,cod_local,obj_ora_s,q_fpv):
    query_trx_fpv=r_q_fp_vale(fecha_i,fecha_f,cod_local)
...
def procesar_local(fecha_actual_f_i,fecha_actual_f_f,cod_local):
    try:
        ...
        if tp_trx:
            ...
            #procesar_fpinterno_df(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_fpi)
            #procesar_fpefectivo_df(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_fpef)
            #procesar_fpvale_df(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_fpv)
            #procesar_promocion_df(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_promo)
            ...
            #d=5/"0a"
            #print(d)
            print(f"Termino de procesamiento Hilo actual: {threading.current_thread().name}")
            obj_ora_s.cerrar()
            return 1,df_cab,df_det,df_fpe,df_fpi,df_fpef,df_fpv,df_pro,fecha_actual_f_i,cod_local
            
        else:
            print(f"Termino de procesamiento Hilo actual: {threading.current_thread().name}")
            print(f"No hay data para cargar en ICT2_TRXS_VTA_BI_GROUP_DIARIO con fechas {fecha_actual_f_i} {fecha_actual_f_f}  y local {cod_local}")
            obj_ora_s.cerrar()
            return 0,False,False,False,False,False,False,False,fecha_actual_f_i,cod_local
    except Exception as e:
        try:
            err=f"{e} para el local {cod_local} con fecha {fecha_actual_f_i}"
            print(err)
            enviar_correo (err)
...
try:
    ...
    table_fpi="tmp_forma_pago_interno"
    table_fpef="tmp_forma_pago_efectivo"
    table_fpv="tmp_forma_pago_vale"
    ...
    procesar_local(fecha_actual_f_i,fecha_actual_f_f,cod_local)
