...
fecha='2024-07-12'
fecha_rep=datetime.datetime.strftime(datetime.datetime.strptime(fecha,"%Y-%m-%d"),"%Y%m%d")
fecha_p=fecha_rep

print(f"Fecha de Carga {fecha}")
...
SELECT distinct l.loc_numero    AS cod_suc_pos
   ...
   and l.cad_numero=i.cad_numero
     ...
     --and l.loc_numero in (783, 789, 792, 793, 794, 795)
     order by 1 
"""
    ...
    and exists
                (SELECT 1
                    ...
                    (
                        ...
                        AND a.hed_fcontable =to_date({fecha_i},'yyyymmdd')
...
def r_q_trx_carga(fecha_i,codigo_local):
    q_trx_carga=f"""
...
def r_q_cabecera(fecha_i,codigo_local):
    q_cabecera=f"""
SELECT /*+ index(h) */ to_char(h.hed_fcontable,'YYYY-MM-DD') AS Fecha_Proceso,
  ...
  WHERE h.hed_pais = b.hed_pais
    ...
    AND b.hed_fcontable = to_date({fecha_i},'yyyymmdd')
    AND b.flg_envvta = 0
    """
    return q_cabecera

def r_q_detalle(fecha_i,codigo_local):
    q_detalle=f"""
   ...
   WHERE  h.hed_pais = p.hed_pais
     ...
     AND h.hed_local = {codigo_local}
    AND h.hed_fcontable = to_date({fecha_i},'yyyymmdd')
    AND h.flg_envvta = 0
    """
    return q_detalle

def r_q_fp_externo(fecha_i,codigo_local):
...
def r_q_fp_interno(fecha_i,codigo_local):
...
def r_q_fp_efectivo(fecha_i,codigo_local):
...
def r_q_fp_vale(fecha_i,codigo_local):
...
def r_q_promocion(fecha_i,codigo_local):
...
def procesar_cab_df(fecha_i,cod_local,obj_ora_s,q_c):
    query_trx_cab=r_q_cabecera(fecha_i,cod_local)
...
def procesar_det_df(fecha_i,cod_local,obj_ora_s,q_d):
    query_trx_det=r_q_detalle(fecha_i,cod_local)
...
def procesar_fpexterno_df(fecha_i,cod_local,obj_ora_s,q_fpe):
    query_trx_fpe=r_q_fp_externo(fecha_i,cod_local)
...
#procesar_fpinterno_df
def procesar_fpinterno_df(fecha_i,cod_local,obj_ora_s,q_fpi):
    query_trx_fpi=r_q_fp_interno(fecha_i,cod_local)
    tp_trx_fpi=obj_ora_s.ejecutar_query(query_trx_fpi)
    df_fpi=pd.DataFrame(data=tp_trx_fpi,columns=[
          "FECHA_PROCESO","CODIGO_LOCAL","NUMERO_TERMINAL","NUMERO_TRANSACCION","CORRELATIVO","CODIGO_PAGO","CODIGO_TIPO_PAGO","NUMERO_CUENTA","IMPORTE_PAGO_BRUTO","NUMERO_RUC","MARCA_BANDA","MARCA_BANDA_MAGNETICA","ID_USUARIO","MARCA_REPACTACION","ESTADO","MARCA_INTERES","CODIGO_AUTORIZACION","CODIGO_MONEDA","BANDA_MAGNETICA","CODIGO_EN_LINEA","CODIGO_TIPO_DOCUMENTO","NUMERO_DOCUMENTO","TIPO_CREDITO","VALOR_CUOTA","LINEA"
    
    ])  
    
    if df_fpi.empty!=True:
        for col in df_fpi.columns:
            df_fpi[col] = df_fpi[col].fillna(valor_reemplazo(df_fpi[col]))
        df_fpi["FECHA_PROCESO"]=df_fpi["FECHA_PROCESO"].map(str)
        df_fpi["CODIGO_LOCAL"]=df_fpi["CODIGO_LOCAL"].map(str)
        df_fpi["NUMERO_TERMINAL"]=df_fpi["NUMERO_TERMINAL"].map(str)
        df_fpi["NUMERO_TRANSACCION"]=df_fpi["NUMERO_TRANSACCION"].map(str)
        df_fpi["CORRELATIVO"]=df_fpi["CORRELATIVO"].map(str)
        df_fpi["CODIGO_PAGO"]=df_fpi["CODIGO_PAGO"].map(str)
        df_fpi["CODIGO_TIPO_PAGO"]=df_fpi["CODIGO_TIPO_PAGO"].map(str)
        df_fpi["NUMERO_CUENTA"]=df_fpi["NUMERO_CUENTA"].map(str)
        df_fpi["IMPORTE_PAGO_BRUTO"]=df_fpi["IMPORTE_PAGO_BRUTO"].map(str)
        df_fpi["NUMERO_RUC"]=df_fpi["NUMERO_RUC"].map(str)
        df_fpi["MARCA_BANDA"]=df_fpi["MARCA_BANDA"].map(str)
        df_fpi["MARCA_BANDA_MAGNETICA"]=df_fpi["MARCA_BANDA_MAGNETICA"].map(str)
        df_fpi["ID_USUARIO"]=df_fpi["ID_USUARIO"].map(str)
        df_fpi["MARCA_REPACTACION"]=df_fpi["MARCA_REPACTACION"].map(str)
        df_fpi["ESTADO"]=df_fpi["ESTADO"].map(str)
        df_fpi["MARCA_INTERES"]=df_fpi["MARCA_INTERES"].map(str)
        df_fpi["CODIGO_AUTORIZACION"]=df_fpi["CODIGO_AUTORIZACION"].map(str)
        df_fpi["CODIGO_MONEDA"]=df_fpi["CODIGO_MONEDA"].map(str)
        df_fpi["BANDA_MAGNETICA"]=df_fpi["BANDA_MAGNETICA"].map(str)
        df_fpi["CODIGO_EN_LINEA"]=df_fpi["CODIGO_EN_LINEA"].map(str)
        df_fpi["CODIGO_TIPO_DOCUMENTO"]=df_fpi["CODIGO_TIPO_DOCUMENTO"].map(str)
        df_fpi["NUMERO_DOCUMENTO"]=df_fpi["NUMERO_DOCUMENTO"].map(str)
        df_fpi["TIPO_CREDITO"]=df_fpi["TIPO_CREDITO"].map(str)
        df_fpi["VALOR_CUOTA"]=df_fpi["VALOR_CUOTA"].map(str)
        df_fpi["LINEA"]=df_fpi["LINEA"].map(str)

        q_fpi.put(df_fpi)
    else:
        return True
    
#procesar_fpefectivo_df
def procesar_fpefectivo_df(fecha_i,cod_local,obj_ora_s,q_fpef):
    query_trx_fpef=r_q_fp_efectivo(fecha_i,cod_local)
    tp_trx_fpef=obj_ora_s.ejecutar_query(query_trx_fpef)
    df_fpef=pd.DataFrame(data=tp_trx_fpef,columns=[
   "FECHA_PROCESO","CODIGO_LOCAL","NUMERO_TERMINAL","NUMERO_TRANSACCION","CORRELATIVO","CODIGO_PAGO","CODIGO_TIPO_PAGO","CODIGO_MONEDA","IMPORTE_PAGO_BRUTO","MONTO_VUELTO","MONTO_REDONDEO","LINEA"
    ])  
    
    if df_fpef.empty!=True:
        for col in df_fpef.columns:
            df_fpef[col] = df_fpef[col].fillna(valor_reemplazo(df_fpef[col]))
        df_fpef["FECHA_PROCESO"]=df_fpef["FECHA_PROCESO"].map(str)
        df_fpef["CODIGO_LOCAL"]=df_fpef["CODIGO_LOCAL"].map(str)
        df_fpef["NUMERO_TERMINAL"]=df_fpef["NUMERO_TERMINAL"].map(str)
        df_fpef["NUMERO_TRANSACCION"]=df_fpef["NUMERO_TRANSACCION"].map(str)
        df_fpef["CORRELATIVO"]=df_fpef["CORRELATIVO"].map(str)
        df_fpef["CODIGO_PAGO"]=df_fpef["CODIGO_PAGO"].map(str)
        df_fpef["CODIGO_TIPO_PAGO"]=df_fpef["CODIGO_TIPO_PAGO"].map(str)
        df_fpef["CODIGO_MONEDA"]=df_fpef["CODIGO_MONEDA"].map(str)
        df_fpef["IMPORTE_PAGO_BRUTO"]=df_fpef["IMPORTE_PAGO_BRUTO"].map(str)
        df_fpef["MONTO_VUELTO"]=df_fpef["MONTO_VUELTO"].map(str)
        df_fpef["MONTO_REDONDEO"]=df_fpef["MONTO_REDONDEO"].map(str)
        df_fpef["LINEA"]=df_fpef["LINEA"].map(str)
...
#procesar_fpvale_df
def procesar_fpvale_df(fecha_i,cod_local,obj_ora_s,q_fpv):
    query_trx_fpv=r_q_fp_vale(fecha_i,cod_local)
    tp_trx_fpv=obj_ora_s.ejecutar_query(query_trx_fpv)
    df_fpv=pd.DataFrame(data=tp_trx_fpv,columns=[
   "FECHA_PROCESO","CODIGO_LOCAL","NUMERO_TERMINAL","NUMERO_TRANSACCION","CORRELATIVO","CODIGO_PAGO","CODIGO_TIPO_PAGO","NUMERO_RUC","NUMERO_VALE","NUMERO_SERIE_VALE","CODIGO_SUPERVISOR","FECHA_VALE_CREDITO","CODIGO_TIPO_VALE","FECHA_PROCESO_NC","CODIGO_LOCAL_NC","NUMERO_TERMINAL_NC","NUMERO_TRANSACCION_NC","LINEA"
])  
    
    if df_fpv.empty!=True:
        for col in df_fpv.columns:
            df_fpv[col] = df_fpv[col].fillna(valor_reemplazo(df_fpv[col]))
        df_fpv["FECHA_PROCESO"]=df_fpv["FECHA_PROCESO"].map(str)
        df_fpv["CODIGO_LOCAL"]=df_fpv["CODIGO_LOCAL"].map(str)
        df_fpv["NUMERO_TERMINAL"]=df_fpv["NUMERO_TERMINAL"].map(str)
        df_fpv["NUMERO_TRANSACCION"]=df_fpv["NUMERO_TRANSACCION"].map(str)
        df_fpv["CORRELATIVO"]=df_fpv["CORRELATIVO"].map(str)
        df_fpv["CODIGO_PAGO"]=df_fpv["CODIGO_PAGO"].map(str)
        df_fpv["CODIGO_TIPO_PAGO"]=df_fpv["CODIGO_TIPO_PAGO"].map(str)
        df_fpv["NUMERO_RUC"]=df_fpv["NUMERO_RUC"].map(str)
        df_fpv["NUMERO_VALE"]=df_fpv["NUMERO_VALE"].map(str)
        df_fpv["NUMERO_SERIE_VALE"]=df_fpv["NUMERO_SERIE_VALE"].map(str)
        df_fpv["CODIGO_SUPERVISOR"]=df_fpv["CODIGO_SUPERVISOR"].map(str)
        df_fpv["FECHA_VALE_CREDITO"]=df_fpv["FECHA_VALE_CREDITO"].map(str)
        df_fpv["CODIGO_TIPO_VALE"]=df_fpv["CODIGO_TIPO_VALE"].map(str)
        df_fpv["FECHA_PROCESO_NC"]=df_fpv["FECHA_PROCESO_NC"].map(str)
        df_fpv["CODIGO_LOCAL_NC"]=df_fpv["CODIGO_LOCAL_NC"].map(str)
        df_fpv["NUMERO_TERMINAL_NC"]=df_fpv["NUMERO_TERMINAL_NC"].map(str)
        df_fpv["NUMERO_TRANSACCION_NC"]=df_fpv["NUMERO_TRANSACCION_NC"].map(str)
        df_fpv["LINEA"]=df_fpv["LINEA"].map(str)

        q_fpv.put(df_fpv)
    else:
        return True
       
def procesar_promocion_df(fecha_i,cod_local,obj_ora_s,q_promo):
    query_trx_promocion=r_q_promocion(fecha_i,cod_local)
...
def procesar_fpi_bq(obj_bq,project,dataset,table_fpi,df_fpi):
    obj_bq.ins_table(project,dataset,table_fpi,df_fpi)
   
def procesar_fpef_bq(obj_bq,project,dataset,table_fpef,df_fpef):
    obj_bq.ins_table(project,dataset,table_fpef,df_fpef)
    
def procesar_fpv_bq(obj_bq,project,dataset,table_fpv,df_fpv):
    obj_bq.ins_table(project,dataset,table_fpv,df_fpv)
...
def procesar_local(fecha_p,cod_local):
    try:
        #print(f"Inicio de procemaiento Hilo actual: {threading.current_thread().name}")
        ...
        query_trx_carga=r_q_trx_carga(fecha_p,cod_local)
        ...
        if tp_trx:
            ...
            obj_ora_s.commit()
            print(f"Inserción exitosa en ICT2_TRXS_VTA_BI_GROUP_DIARIO con fechas {fecha_p}  y local {cod_local}")
            ...
            procesar_det_df(fecha_p,cod_local,obj_ora_s,q_d)
            procesar_cab_df(fecha_p,cod_local,obj_ora_s,q_c)
            
            procesar_fpexterno_df(fecha_p,cod_local,obj_ora_s,q_fpe)
            procesar_fpinterno_df(fecha_p,cod_local,obj_ora_s,q_fpi)
            procesar_fpefectivo_df(fecha_p,cod_local,obj_ora_s,q_fpef)
            procesar_fpvale_df(fecha_p,cod_local,obj_ora_s,q_fpv)
            procesar_promocion_df(fecha_p,cod_local,obj_ora_s,q_promo)
            ...
            thread_fpi = ThreadWithException(target=procesar_fpinterno_df, args=(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_fpi))
            thread_fpef = ThreadWithException(target=procesar_fpefectivo_df, args=(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_fpef))
            thread_fpv = ThreadWithException(target=procesar_fpvale_df, args=(fecha_actual_f_i,fecha_actual_f_f,cod_local,obj_ora_s,q_fpv))
            ...
            threads_l.append(thread_fpi)
            threads_l.append(thread_fpef)
            threads_l.append(thread_fpef)
            threads_l.append(thread_fpv)
            ...
            """
            df_cab=q_c.get()
            print(f"datos cabecera cargado a Dataframe para el dia {fecha_p} y local {cod_local}")
            df_det=q_d.get()
            print(f"datos detalle cargado a Dataframe para el dia {fecha_p} y local {cod_local}")
            df_fpe=pd.DataFrame()
            
            df_fpi=pd.DataFrame()
            df_fpef=pd.DataFrame()
            df_fpv=pd.DataFrame()
            df_pro=pd.DataFrame()

            if not q_fpe.empty():
                df_fpe=q_fpe.get()
                print(f"datos forma pago externo cargado a Dataframe para el dia {fecha_p} y local {cod_local}")

            if not q_fpi.empty():
                df_fpi=q_fpi.get()
                print(f"datos forma pago interno cargado a Dataframe para el dia {fecha_p} y local {cod_local}")

            if not q_fpef.empty():
                df_fpef=q_fpef.get()
                print(f"datos forma pago efectivo cargado a Dataframe para el dia {fecha_p} y local {cod_local}")
            
            if not q_fpv.empty():
                df_fpv=q_fpv.get()
                print(f"datos forma pago vale a Dataframe para el dia {fecha_p} y local {cod_local}")

            if not q_promo.empty():
                df_pro=q_promo.get()
                print(f"datos promocion cargado a Dataframe para el dia {fecha_p} y local {cod_local}")

            obj_ora_s.cerrar()
            return 1,df_cab,df_det,df_fpe,df_fpi,df_fpef,df_fpv,df_pro,fecha_p,cod_local
            
        else:
            print(f"No hay data para cargar en ICT2_TRXS_VTA_BI_GROUP_DIARIO con fechas {fecha_p} y local {cod_local}")
            obj_ora_s.cerrar()
            return 0,False,False,False,False,False,False,False,fecha_p,cod_local
    except Exception as e:
        try:
            err=f"{e} para el local {cod_local} con fecha {fecha_p}"
            print(err)
            enviar_correo (err)
            print(f"Error al procesar la data de los dias {fecha_p} y local {cod_local}")
            fecha_actual_f_s_i=datetime.datetime.strptime(datetime.datetime.strftime(datetime.datetime.strptime(fecha_p,"%Y%m%d"),"%d/%m/%Y"),"%d/%m/%Y")
            ...
            obj_ora_s.commit()
            return 9999,False,False,False,False,False,False,False,False,False
        except Exception as e:
            print(e)
            return 9999,False,False,False,False,False,False,False,False,False
...
try:
    ...
    table_cab="tmp_rep_ticket_cabecera"
    table_det="tmp_rep_ticket_detalle"

    table_fpe="tmp_rep_forma_pago_externo"
    table_fpi="tmp_rep_forma_pago_interno"
    table_fpef="tmp_rep_forma_pago_efectivo"
    table_fpv="tmp_rep_forma_pago_vale"

    table_promo="tmp_rep_promocion_detalle"
    ...
    #obj_bq.clear_table(project,dataset,table_fpi)
    #obj_bq.clear_table(project,dataset,table_fpef)
    #obj_bq.clear_table(project,dataset,table_fpv)
    ...
    df_fpi_acum=pd.DataFrame(columns=[
        "FECHA_PROCESO","CODIGO_LOCAL","NUMERO_TERMINAL","NUMERO_TRANSACCION","CORRELATIVO","CODIGO_PAGO","CODIGO_TIPO_PAGO","NUMERO_CUENTA","IMPORTE_PAGO_BRUTO","NUMERO_RUC","MARCA_BANDA","MARCA_BANDA_MAGNETICA","ID_USUARIO","MARCA_REPACTACION","ESTADO","MARCA_INTERES","CODIGO_AUTORIZACION","CODIGO_MONEDA","BANDA_MAGNETICA","CODIGO_EN_LINEA","CODIGO_TIPO_DOCUMENTO","NUMERO_DOCUMENTO","TIPO_CREDITO","VALOR_CUOTA","LINEA"
    ])
    df_fpef_acum=pd.DataFrame(columns=[
        "FECHA_PROCESO","CODIGO_LOCAL","NUMERO_TERMINAL","NUMERO_TRANSACCION","CORRELATIVO","CODIGO_PAGO","CODIGO_TIPO_PAGO","CODIGO_MONEDA","IMPORTE_PAGO_BRUTO","MONTO_VUELTO","MONTO_REDONDEO","LINEA"
    ])
    df_fpv_acum=pd.DataFrame(columns=[
        "FECHA_PROCESO","CODIGO_LOCAL","NUMERO_TERMINAL","NUMERO_TRANSACCION","CORRELATIVO","CODIGO_PAGO","CODIGO_TIPO_PAGO","NUMERO_RUC","NUMERO_VALE","NUMERO_SERIE_VALE","CODIGO_SUPERVISOR","FECHA_VALE_CREDITO","CODIGO_TIPO_VALE","FECHA_PROCESO_NC","CODIGO_LOCAL_NC","NUMERO_TERMINAL_NC","NUMERO_TRANSACCION_NC","LINEA"
   ])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=c) as executor:
        resultados = [executor.submit(procesar_local,fecha_p,i) for i in ls_loc_t]

    insp=0
    for resultado in concurrent.futures.as_completed(resultados):
        f,dfc,dfd,dfpe,dfpi,dfpef,dfpv,dfp,fecha,codigo_local=resultado.result()
    #f,dfc,dfd,dfpe,dfpi,dfpef,dfpv,dfp,fecha,codigo_local=procesar_local(fecha_actual_f_i,768)
        if f==1:
            ...
            if dfpi.empty!=True:
                #df_fpi_acum=pd.concat([df_fpi_acum,dfpi])
                list_fi.append(dfpi)
            if dfpef.empty!=True:
                #df_fpef_acum=pd.concat([df_fpef_acum,dfpef])
                list_fef.append(dfpef)
            if dfpv.empty!=True:
                #df_fpv_acum=pd.concat([df_fpv_acum,dfpv])
                list_fv.append(dfpv)

            if dfp.empty!=True:
                #df_promo_acum=pd.concat([df_promo_acum,dfp])
                list_pd.append(dfp)
            print(f"Fecha {fecha} y local {codigo_local} terminado y cargado en DataFrame")
        else:
            print(f"Fecha {fecha} y local {codigo_local} terminado y cargado en DataFrame")
        insp=insp+1
        print(f"El total es:{insp}")

    if list_c:
        df_cab_acum=pd.concat(list_c,ignore_index=True)
    if list_d:
        df_det_acum=pd.concat(list_d,ignore_index=True)
    if list_fe:
        df_fpe_acum=pd.concat(list_fe,ignore_index=True)
    if list_fi:
        df_fpi_acum=pd.concat(list_fi,ignore_index=True)
    if list_fef:
        df_fpef_acum=pd.concat(list_fef,ignore_index=True)
    if list_fv:
        df_fpv_acum=pd.concat(list_fv,ignore_index=True)
    if list_pd:
        df_promo_acum=pd.concat(list_pd,ignore_index=True)
    ...
    if df_cab_acum.empty == False and df_det_acum.empty == False :
        ...
        if df_fpi_acum.empty!=True:
            pbqfpi = ThreadWithException(target=procesar_fpi_bq,args=(obj_bq,project,dataset,table_fpi,df_fpi_acum))
            pbqfpi.start()
            threads_df.append(pbqfpi)
        
        if df_fpef_acum.empty!=True:
            pbqfpef = ThreadWithException(target=procesar_fpef_bq,args=(obj_bq,project,dataset,table_fpef,df_fpef_acum))
            pbqfpef.start()
            threads_df.append(pbqfpef)
        
        if df_fpv_acum.empty!=True:
            pbqfpv = ThreadWithException(target=procesar_fpv_bq,args=(obj_bq,project,dataset,table_fpv,df_fpv_acum))
            pbqfpv.start()
            threads_df.append(pbqfpv)
        ...
        obj_ora_p.ejecutar_query_sin_return(r_q_upd_tabla_carga(fecha_p))
        obj_ora_p.commit()
