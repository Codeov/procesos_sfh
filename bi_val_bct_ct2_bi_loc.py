...
ruta_archivo_excel_loc=r'D:\dif_local_ct2_bi.xlsx'
...
start_month_cubo = month_now - relativedelta(months=2)#datetime.date(2023,12,25)
...
def ancho_columna_excel(ruta):

  val_file=os.path.exists(ruta)

  if val_file:
    informe = openpyxl.load_workbook(ruta)
...
def carga_locales_ct2():
 query_locales_ct2="""
  select distinct a.loc_numero codigo_local_pmm,a.loc_descripcion descripcion_local,b.cad_descripcion formato
  from irs_locales a
  inner join irs_cadenas b
  on a.cad_numero=b.cad_numero
  where a.loc_numero < 5000
  and b.cad_descripcion not in ('CONVENIENCIA','CD ORIENTE VIRTUAL','CD')
  """
 ...
 df_loc_ct2=extension_oracle.ejecutar_query(query_locales_ct2)
 dfv_loc_ct2=pd.DataFrame(data=df_loc_ct2,columns=["codigo_local_pmm","descripcion_local","formato"])
 dfv_loc_ct2['codigo_local_pmm'] = dfv_loc_ct2["codigo_local_pmm"].map(str)
 dfv_loc_ct2['descripcion_local'] = dfv_loc_ct2["descripcion_local"].map(str)
 dfv_loc_ct2['formato'] = dfv_loc_ct2["formato"].map(str)
...
def sp_procesar_dif_bct_ct2_bi(fecha_inicial,fecha_final):
  try:
    fecha_i=str(fecha_inicial)
    fecha_f=str(fecha_final)
    ins_pbi=bigquery.Client.from_service_account_json(ruta_json_pbi)
    request_pbi=ins_pbi.query(f"call `spsa-sistemasbi-powerbi.BI_VALIDADORES.sp_procesar_dif_bct_ct2_bi` ('{fecha_i}','{fecha_f}')",location="us")
    request_pbi.result()
...
def sp_procesar_dif_ct2_bi_loc (fecha_inicial,fecha_final):
  try:
    fecha_i=str(fecha_inicial)
    fecha_f=str(fecha_final)
    ins_pbi=bigquery.Client.from_service_account_json(ruta_json_pbi)
    request_pbi=ins_pbi.query(f"call `spsa-sistemasbi-powerbi.BI_VALIDADORES.sp_procesar_dif_ct2_bi_loc` ('{fecha_i}','{fecha_f}')",location="us")
    request_pbi.result()
    print(" Se terminó de procesar las diferencias de CT2 y BI nivel local en Big Query")
  except Exception as e:
    print(e)
    raise Exception ("Error al procesar las diferencias de CT2 y BI nivel local en Big Query")
...
def df_tabla_val_loc ():

  query_ct2_bi_loc="""
  select a.fecha,a.codigo_alterno codigo_local_pmm,a.venta_neta_ct2,a.venta_neta_bi,a.dif diferencia
  from `spsa-sistemasbi-powerbi.BI_VALIDADORES.dif_venta_neta_bi_ct2_local` a
  where a.dif not between -1 and 1
  order by 5 desc
  """
  req_clt=bigquery.Client.from_service_account_json(ruta_json_pbi)
  request_ct2_bi_loc=req_clt.query(query_ct2_bi_loc,location="us")
  request_ct2_bi_loc.result()
  df_ct2_bi_loc=request_ct2_bi_loc.to_dataframe()
  return df_ct2_bi_loc

def generar_file_ct2_bi_loc(df_ct2_bi_loc):

  df=pd.DataFrame(data=df_ct2_bi_loc,columns=["fecha","formato","codigo_local_pmm","descripcion_local","venta_neta_ct2","venta_neta_bi","diferencia"])
  df['fecha']=df['fecha'].map(str)
  df['formato']=df['formato'].map(str)
  df['codigo_local_pmm']=df['codigo_local_pmm'].map(str)
  df['descripcion_local']=df['descripcion_local'].map(str)
  df['venta_neta_ct2']=df['venta_neta_ct2'].map(float)
  df['venta_neta_bi']=df['venta_neta_bi'].map(float)
  df['diferencia']=df['diferencia'].map(float)
  df.to_excel(excel_writer=ruta_archivo_excel_loc,index=False,sheet_name='dif_loc_ct2_bi')
  ancho_columna_excel(ruta_archivo_excel_loc)
  return True

def val_data_bct_ct2_bi():
 df_ct2_bi=df_tabla_val("ct2_bi")
 df_bct_ct2_bi=df_tabla_val("bct_ct2_bi")
 if df_bct_ct2_bi.empty!=True: 
    flag_val_p="bct_ct2_bi"
    return flag_val_p,df_bct_ct2_bi,df_ct2_bi
 elif df_bct_ct2_bi.empty==True: 
...
def val_load_html(flag_val_p,df_p,flag_val_s,df_s,hora,flag_val_ct2_bi_loc):
  if df_p.empty!=True:
  ...
  aviso=f"""
              <p>Estimados su apoyo con la coordinacion y reproceso urgente.</p>
               """
  aviso_archivo=f"""
              <p>Estimados su apoyo con la coordinacion y reproceso urgente, asimismo se adjunta archivo con los locales que tienen diferencia entre CT2 & BI</p>
               """
  footer=f"""
              <center>
  ...
  if flag_val_p=="bct_ct2_bi" and flag_val_s=="tv_az" and flag_val_ct2_bi_loc==1:
    html=f"""
          <html>
            <head>
            </head>
    ...
    print(html)
    return html
  elif flag_val_p=="bct_ct2_bi" and flag_val_s=="not_tv_az" and flag_val_ct2_bi_loc==1:
    html=f"""
          <html>
            <head>
            </head>
    ...
    print(html)
    return html
  elif flag_val_p=="bct_ct2_bi" and flag_val_s=="tv_az":
    html=f"""
          <html>
            <head>
            </head>
...
def enviar_correo (html,flag_val_ct2_bi_loc):
  try:
    ...
    if flag_val_ct2_bi_loc==1:
      with open(ruta_archivo_excel_loc, "rb") as f:
        email.add_attachment(
          f.read(),
          filename="dif_venta_loc_ct2_bi.xlsx",
...
sp_procesar_dif_bct_ct2_bi(start_month_cubo,end_month_cubo)

hora_actual=hora_actual()

flag_val_p,df_p,df_ct2_bi=val_data_bct_ct2_bi()
flag_val_s,df_s=val_data_tv_az()
flag_val_ct2_bi_loc=0

if df_ct2_bi.empty!=True:
  sp_procesar_dif_ct2_bi_loc(start_month_cubo,end_month_cubo)
  df_ct2_bi_loc=df_tabla_val_loc()
  df_ct2_loc=carga_locales_ct2()
  if df_ct2_bi_loc.empty!=True and df_ct2_loc.empty!=True:
    df_merge=pd.merge(df_ct2_bi_loc,df_ct2_loc,how='left',on='codigo_local_pmm')
    df_ct2_bi_loc_final=pd.DataFrame(data=df_merge,columns=["fecha","formato","codigo_local_pmm","descripcion_local","venta_neta_ct2","venta_neta_bi","diferencia"]) 
    generar_file_ct2_bi_loc(df_ct2_bi_loc_final)
    flag_val_ct2_bi_loc=1
  else:
    raise Exception("Error en el proceso de diferencia locales CT2 & BI")

html=val_load_html(flag_val_p,df_p,flag_val_s,df_s,hora_actual,flag_val_ct2_bi_loc)

if html !=None:
  enviar_correo(html,flag_val_ct2_bi_loc)
